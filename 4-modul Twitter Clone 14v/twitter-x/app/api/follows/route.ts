...
export async function DELETE(req: Request) {
    try {
        ...
        await User.findByIdAndUpdate(userId, {
            $pull: { followers: currentUserId }, //pullni pushdan farqi pull udalit qiladi push esa qo'shadi bu degani bu DELETE  functionida kelgan mongoosesini findByIdAndUpdate functionini pull qiymati user modeldagi userni folllowers bo'limiga borib turgan current userni idisni userIddan olib udalit qiladi qachon udalit qiladi profile-bio.tsx faildagi onUnfollow functionidagi axios so'rov bajarilganda yani onUnfollow functionga click bo'lganda sodir bo'ladi va shu bilan bir vaqtda pastdagi findByIdAndUpdateham ishlab currentuerid ichidagi following objectini ichidan currenuseridni useIdsini olin followingdan pull qiladi yani bir faqtni o'zida bitta so'rovni o'zida shu ikkala function ishlab profile-bioda  onUnFollow functioni berilgan button ishlaganda ishlaydi yani folllowni unfollow qilib current userdanham oddiy turgan userdanham ikkala following va followersdan udalit qiladi
        });
...
export async function GET(req: Request) {
    //bu get so'rov profile-bio.tsx failida getFollowUser, openFollowModal, openFollowModal, onFollowers functionlarida axios bilan chaqirilgan
    try {
        ...
        if (state === "following") {
            ...
            //yokida followerslar jo'natiladi
            const followers = await User.find({ _id: { $in: user.followers } });

            return NextResponse.json(followers); //qaytaradi followersni
        }
    } catch (error) {
        const result = error as Error;
        return NextResponse.json({ error: result.message }, { status: 400 });
