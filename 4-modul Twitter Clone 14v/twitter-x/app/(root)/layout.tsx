...
const Layout = async ({ children }: Props) => {//bu ts shu sabab componentgaham nima ekanligini typi aytib qo'yilish kerak bu layoutda children ishlatiladi shusabab typi aytib qo'yilgan interfaceda yani reactnode elementlar
    const session: any = await getServerSession(authOptions);//nextauthni server bilan ishlaydigan functioni 

    // console.log(session.currentUser);

    if (!session) {//yani agar session yo'q bo'lsa yani session o'zgaruvchidagi getServerSession bilan chaqirilgan authOptions false bo'lsa yani user ro'yhatdan o'tish jaroyonini tugatmagan bo'lsa yani authOptions functioni ishlab turgan  bo'lsa Auth pageni return qiladi auth pageda esa loginmodal registermodallar chaqirilgan shunda authdagi return ishga tushib user royhatdan o'tishi kerak bo'lgan sahifa yana actual qoladi yokida pastdagi returnni qaytaradi yaniasosiy sahifaga olib chiqadi yani session strue bo'lsa yani user ro'yhatdan o'tib bo'lgan yoki login paswwordlarni togri kiritgan bo'lsa pastdagiu return aftamatik tarzda ishlab userni asosiy sahifaga olib chiqadi
        return (
