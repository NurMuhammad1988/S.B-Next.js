...
export const useScrolled = () => {
    ...
    useEffect(() => {
        ...
        window.addEventListener("scroll", handleScroll);// jsni addEventListener metodiga yuqoridagi handleScroll functioni ulab qo'yildi yani addEventListener metodini scroll qiymatiga ulab qo'yildi
        return () => window.removeEventListener("scroll", handleScroll);//yani return doim addEventListener metodini udalit qiladi yani if else ishlagandan keyin hodisa yopiladi ishlashdan to'htaydi yani mishka qimillaganda ishlaydi qimillamaganda removeEventListener sabab udalit bo'ladi
    }, []);
    return scrolled;
};
