...
const Page = async ({ params }: { params: { userId: string } }) => {
    ...
    return (
        <>
            ...
            {/* //lib/actions/user.action.tsdan chaqirilgan user */}
            {/* ProfileHero componentga userni json qilib jo'natish chunki user serverdan rendring bo'lib kelepti serverdan keletgan failni js tanimaydi shu sabab json qilish kerak */}

            <ProfileBio
                user={JSON.parse(JSON.stringify(user))}//yani obshi user//bu obshi userlarni ichidan current user const session: any = await getServerSession(authOptions); bilan ajratib olinadi 
                userId={JSON.parse(JSON.stringify(session)).currentUser._id} //currentUser lib/actions/user.action.tsdan params sabab kelgan o'zgaruvchi ichida idilar bor yani bitta id bor yani activ id//getUserById functioni  getServerSession sabab ishlaydi va currentUserda kelgan idni json formatga o'giradi
            />
